...
export (float) var water_top = 0.2
var area: Area2D
...
func _ready():
	material.set_shader_param("u_scale", scale)
	material.set_shader_param("u_water_top", water_top)
...
func _on_body_shape_entered(body_id, body, body_shape, area_shape):
	...
	# 传递给Shader
	material.set_shader_param("u_entry_point", entry_point)
	material.set_shader_param("u_enter_time", OS.get_ticks_msec() / 1000)
